...
public class MainApp extends Application {
    ...
    public static class UsuarioService {
        ...
        private void cargar() {
            try (BufferedReader br = new BufferedReader(new FileReader(ARCHIVO))) {
                ...
                usuarios.add(new Usuario("admin", "admin", "admin", "Administrador", "Sistema", "0000000000", "admin@stocksync.com"));
                usuarios.add(new Usuario("cliente", "cliente", "cliente", "Cliente", "Ejemplo", "1234567890", "cliente@example.com"));
                guardar();
            }
        }
    ...
    private void configurarIcono(Stage stage) {
        ...
        if (icono != null) stage.getIcons().add(icono);
    }

    /** Aplica una imagen de fondo como Background a un Region (comportamiento original). */
    ...
    /** Envuelve el contenido en un StackPane con imagen de fondo que se ajusta al tamaño de la ventana. */
    private StackPane crearFondo(Node contenido, String rutaClasspath) {
        Image img = cargarImagen(rutaClasspath);
        if (img == null) return new StackPane(contenido);
    ...
    private void mostrarAdmin() {
        ...
        TextField addNombres = new TextField(); addNombres.setPromptText("Nombres");
        TextField addApellidos = new TextField(); addApellidos.setPromptText("Apellidos");
        TextField addCedula = new TextField(); addCedula.setPromptText("Cédula");
        TextField addEmail = new TextField(); addEmail.setPromptText("Correo Electrónico");
        TextField addUser = new TextField(); addUser.setPromptText("Nombre de Usuario");
        PasswordField addPass = new PasswordField(); addPass.setPromptText("Contraseña");
        ComboBox<String> addRol = new ComboBox<>();
        addRol.getItems().addAll("cliente", "admin");
        addRol.setValue("cliente");

        addGrid.add(new Label("Nombres:"), 0, 0); addGrid.add(addNombres, 1, 0);
        addGrid.add(new Label("Apellidos:"), 0, 1); addGrid.add(addApellidos, 1, 1);
        addGrid.add(new Label("Cédula:"), 0, 2); addGrid.add(addCedula, 1, 2);
        addGrid.add(new Label("Email:"), 0, 3); addGrid.add(addEmail, 1, 3);
        addGrid.add(new Label("Usuario:"), 0, 4); addGrid.add(addUser, 1, 4);
        addGrid.add(new Label("Contraseña:"), 0, 5); addGrid.add(addPass, 1, 5);
        addGrid.add(new Label("Rol:"), 0, 6); addGrid.add(addRol, 1, 6);
        ...
        Label hu1 = new Label("Usuario"); hu1.setStyle(bold);
        Label hu2 = new Label("Nombres"); hu2.setStyle(bold);
        Label hu3 = new Label("Apellidos"); hu3.setStyle(bold);
        Label hu4 = new Label("Cédula"); hu4.setStyle(bold);
        Label hu5 = new Label("Email"); hu5.setStyle(bold);
        Label hu6 = new Label("Rol"); hu6.setStyle(bold);
        ...
        btnAddUser.setStyle("-fx-background-color:#007bff; -fx-text-fill:white; -fx-font-weight:bold; -fx-padding:8 15;");
        ...
        btnAddUser.setOnAction(e -> {
            ...
            if (n.isEmpty() || a.isEmpty() || c.isEmpty() || em.isEmpty() || u.isEmpty() || p.isEmpty() || rol == null) {
                mostrarAlerta(Alert.AlertType.WARNING, "Complete todos los campos");
                return;
            }
            ...
            addNombres.clear(); addApellidos.clear(); addCedula.clear();
            addEmail.clear(); addUser.clear(); addPass.clear();
            addRol.setValue("cliente");
