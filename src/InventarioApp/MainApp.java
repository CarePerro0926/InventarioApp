...
public class MainApp extends Application {
    ...
    public static class Producto {
        ...
        public String toString() {
            ...
            return "ID: " + id + ", Nombre: " + nombre + ", Cantidad: " + cantidad + ", Precio por unidad: "
                    + formatoPesos.format(precio) + " COP";
        }
        ...
        private void cargarDatosDesdeArchivo() {
            ...
            try (BufferedReader br = new BufferedReader(new FileReader(ARCHIVO_INVENTARIO))) {
                ...
                while ((linea = br.readLine()) != null) {
                    ...
                    if (datos.length == 4) {
                        inventario.add(new Producto(datos[0], datos[1], Integer.parseInt(datos[2]),
    ...
    public void start(Stage primaryStage) {
        ...
        btnLogin.setStyle(
                "-fx-background-color:linear-gradient(#2e4053,#2e4053); -fx-background-radius:10; -fx-text-fill:white; -fx-font-size:12px; -fx-font-weight:bold; -fx-padding:8 15; -fx-effect:dropshadow(three-pass-box,rgba(0,0,0,0.3),5,0,0,2);");
        ...
        btnLogin.setOnAction(e -> {
            ...
            if (usuarioService.validarUsuario(username, password)) {
                ...
                mostrarAlertaAutoCierre(Alert.AlertType.ERROR, "Credenciales inválidas. Intente nuevamente.",
                        FONDO_PATH);
            }
        });
    ...
    private void abrirVentanaRegistro(UsuarioService usuarioService) {
        ...
        root.getChildren().addAll(new Label("Ingrese sus datos:"), txtNombres, txtApellidos, txtCedula, datePicker,
                txtCorreo, txtNombredeUsuario, btnGuardar, btnCancelar);
        ...
        btnGuardar.setOnAction(e -> {
            ...
            if (nombres.isEmpty() || apellidos.isEmpty() || cedula.isEmpty() || fechaNacimiento == null
                    || correo.isEmpty() || nombreUsuario.isEmpty()) {
                ...
                mostrarAlertaAutoCierre(Alert.AlertType.INFORMATION, "Usuario registrado exitosamente.",
                        "file:/D:/path/to/chequeo_icon.png");
                registroStage.close();
            }
        });
    ...
    private void abrirCatalogo(Stage parentStage) {
        ...
        hId.setStyle(bold);
        Label hName = new Label("Nombre");
        hName.setStyle(bold);
        Label hDisp = new Label("Cantidad disponible");
        hDisp.setStyle(bold);
        Label hPrecio = new Label("Precio unidad");
        hPrecio.setStyle(bold);
        ...
        for (int row = 1; row <= inventarioService.obtenerInventario().size(); row++) {
            ...
            Label cId = new Label("ID: " + p.getId());
            cId.setTextFill(Color.RED);
            cId.setStyle(bold);
            Label cName = new Label(p.getNombre());
            Label cDisp = new Label("" + p.getCantidad());
            GridPane.setHalignment(cDisp, HPos.CENTER);
            Label cPrecio = new Label(df.format(p.getPrecio()) + " COP");
            grid.add(cId, 0, row);
            grid.add(cName, 1, row);
            grid.add(cDisp, 2, row);
            grid.add(cPrecio, 3, row);
        }
        ...
        txtSearch.textProperty().addListener((obs, ov, nv) -> {
            ...
            int r = 1;
            for (Producto p : list) {
                Label iLbl = new Label("ID: " + p.getId());
                iLbl.setTextFill(Color.RED);
                iLbl.setStyle(bold);
                Label nLbl = new Label(p.getNombre());
                Label dLbl = new Label("" + p.getCantidad());
                GridPane.setHalignment(dLbl, HPos.CENTER);
    ...
    private void abrirAdmin() {
        ...
        root.setPadding(new Insets(10));
        aplicarFondoResponsive(root,
                "D:/ARCHIVOS UNIVERSIDAD/PROYECTOS ECLIPSE/InventarioApp/src/InventarioApp/images/inventario_fondo.png");
        ...
        centerBox.getChildren().addAll(btnConsultarInventario, btnAgregarProducto, btnEliminarProducto,
                btnActualizarInventario, btnActualizarProveedores, btnConsultarProveedores);
    ...
    private void abrirVentanaConsultarProveedores() {
        ...
        Image imagenFondo = new Image(
                "file:/D:/ARCHIVOS UNIVERSIDAD/PROYECTOS ECLIPSE/InventarioApp/src/InventarioApp/images/inventario_fondo.png");
        BackgroundImage fondo = new BackgroundImage(imagenFondo, BackgroundRepeat.NO_REPEAT, BackgroundRepeat.NO_REPEAT,
                BackgroundPosition.CENTER, new BackgroundSize(400, 300, false, false, false, false));
    ...
    private void abrirVentanaConsultarInventario() {
        ...
        for (Producto p : inventarioService.obtenerInventario()) {
            ...
            Label lblDisp = new Label(String.valueOf(p.getCantidad()));
            GridPane.setHalignment(lblDisp, HPos.CENTER);
            Label lblPrecio = new Label(df.format(p.getPrecio()) + " COP");
            grid.add(lblId, 0, row);
            grid.add(lblName, 1, row);
            grid.add(lblDisp, 2, row);
            grid.add(lblPrecio, 3, row);
            row++;
        }
        ...
        txtSearch.textProperty().addListener((obs, oldVal, newVal) -> {
            ...
            for (Producto p : filtrados) {
                ...
                idLbl.setTextFill(Color.RED);
                idLbl.setStyle(bold);
                Label nameLbl = new Label(p.getNombre());
                Label dispLbl = new Label(String.valueOf(p.getCantidad()));
                GridPane.setHalignment(dispLbl, HPos.CENTER);
                Label priceLbl = new Label(df.format(p.getPrecio()) + " COP");
                grid.add(idLbl, 0, f);
                grid.add(nameLbl, 1, f);
                grid.add(dispLbl, 2, f);
                grid.add(priceLbl, 3, f);
                f++;
            }
        });
    ...
    private void abrirVentanaAgregarProducto() {
        ...
        Image imagenFondo = new Image(
                "file:/D:/ARCHIVOS UNIVERSIDAD/PROYECTOS ECLIPSE/InventarioApp/src/InventarioApp/images/inventario_fondo.png");
        BackgroundImage fondo = new BackgroundImage(imagenFondo, BackgroundRepeat.NO_REPEAT, BackgroundRepeat.NO_REPEAT,
                BackgroundPosition.CENTER, new BackgroundSize(300, 300, false, false, false, false));
        ...
        btnEliminar.setOnAction(e -> {
            ...
            if (seleccionado == null) {
                ...
                Alert info = new Alert(Alert.AlertType.INFORMATION,
                        "Producto \"" + seleccionado.getNombre() + "\" eliminado.");
    ...
    private void abrirVentanaActualizarInventario() {
        ...
        Image imagenFondo = new Image(
                "file:/D:/ARCHIVOS UNIVERSIDAD/PROYECTOS ECLIPSE/InventarioApp/src/InventarioApp/images/inventario_fondo.png");
        BackgroundImage fondo = new BackgroundImage(imagenFondo, BackgroundRepeat.NO_REPEAT, BackgroundRepeat.NO_REPEAT,
                BackgroundPosition.CENTER, new BackgroundSize(300, 250, false, false, false, false));
        actualizarRoot.setBackground(new Background(fondo));

        TextField txtId = new TextField();
        txtId.setPromptText("ID del Producto a Actualizar");
        TextField txtCantidad = new TextField();
        txtCantidad.setPromptText("Nueva Cantidad");
        TextField txtPrecio = new TextField();
    ...
    private void abrirVentanaActualizarProveedores() {
        ...
        Image imagenFondo = new Image(
                "file:/D:/ARCHIVOS UNIVERSIDAD/PROYECTOS ECLIPSE/InventarioApp/src/InventarioApp/images/inventario_fondo.png");
        BackgroundImage fondo = new BackgroundImage(imagenFondo, BackgroundRepeat.NO_REPEAT, BackgroundRepeat.NO_REPEAT,
                BackgroundPosition.CENTER, new BackgroundSize(300, 200, false, false, false, false));
        proveedoresRoot.setBackground(new Background(fondo));

        TextField txtNombre = new TextField();
        txtNombre.setPromptText("Nombre del Proveedor");
        TextField txtContacto = new TextField();
        txtContacto.setPromptText("Contacto del Proveedor");
    ...
    private void abrirCliente() {
        ...
        Label hName = new Label("Nombre");
        hName.setStyle(bold);
        Label hDisp = new Label("Disponible");
        hDisp.setStyle(bold);
        ...
        for (Producto p : inventarioService.obtenerInventario()) {
            ...
            grid.add(lblName, 1, row);
            grid.add(lblDisp, 2, row);
            grid.add(tfCantidad, 3, row);
            grid.add(lblPrecio, 4, row);
            row++;
        }
        ...
        txtSearch.textProperty().addListener((obs, oldVal, newVal) -> {
            ...
            for (Producto p : filtrados) {
                ...
                grid.add(dispLbl, 2, f);
                grid.add(tf, 3, f);
                grid.add(priceLbl, 4, f);
                f++;
            }
        });

        Button btnPagar = new Button("Pagar");
        btnPagar.setOnAction(e -> abrirVentanaPago());
        btnPagar.setStyle(
                "-fx-background-color: linear-gradient(#2e4053,#2e4053); -fx-text-fill: white; -fx-font-size: 12px; -fx-font-weight: bold; -fx-padding: 8 15 8 15;");
        ...
        HBox totalBox = new HBox(totalFlow);
        totalBox.setAlignment(Pos.CENTER_LEFT);
        HBox pagarBox = new HBox(btnPagar);
        pagarBox.setAlignment(Pos.CENTER_LEFT);
    ...
    // ================= MODIFICACIÓN CLAVE: DESCUENTO DE INVENTARIO
    // =================
    ...
    /**
     * Descuenta las cantidades del carrito del inventario principal
     * 
     * @return true si todo el stock fue descontado, false si no hay suficiente
     *         stock
     */
        ...
        for (Producto itemCarrito : carrito) {
            for (Producto productoInventario : inventarioService.obtenerInventario()) {
                if (productoInventario.getId().equals(itemCarrito.getId())) {
                    ...
                    // Actualizar en el inventario (esto también guarda automáticamente en el
                    // archivo .txt)
